...
R_htdf = model.addVars(hogares.keys(), dias, fiscalizadores, vtype=GRB.BINARY, name="R_htdf")
L_htdo = model.addVars(hogares.keys(), dias, operadores, vtype=GRB.BINARY, name="L_htdo")
V_tdfky = model.addVars(dias, fiscalizadores, comunas_santiago.keys(), comunas_santiago.keys(), vtype=GRB.BINARY, name="V_tdfky")
PCR_htdf =  model.addVars(hogares.keys(), dias, fiscalizadores, vtype=GRB.BINARY, name="PCR_htdf")
ANT_htdf = model.addVars(hogares.keys(), dias, fiscalizadores, vtype=GRB.BINARY, name="ANT_htdf")
P_tdfs = model.addVars(dias, fiscalizadores, comunas_santiago.keys(), vtype=GRB.BINARY, name="P_tdfs")
...
model.addConstrs((quicksum(L_htdo[hogar, dia, operador] for operador in operadores) <=
...
model.addConstrs((quicksum(R_htdf[hogar, dia, fiscalizador] for hogar in hogares.keys() for fiscalizador in fiscalizadores) 
                  * alpha[implemento] <= I_ad[implemento, dia] 
...
# 8) Flujo inventario
model.addConstrs((quicksum(R_htdf[hogar, dia, fiscalizador] for hogar in hogares.keys() for fiscalizador in fiscalizadores) 
                  * alpha[implemento] + iI_ad[implemento, dia] <= 
...
model.addConstrs((quicksum(L_htdo[hogar, dia, operador] for hogar in hogares.keys()) <= 
                  maxc for operador in operadores for dia in dias), name="max_llamadas_ph")

# 16) Restriccion fiscalizacion cada 2 dias maximo
model.addConstrs((quicksum(R_htdf[hogar, dia, fiscalizador] + R_htdf[hogar, hora-1, dia, fiscalizador] 
...
model.addConstrs((quicksum(PCR_htdf[hogar, hora, dia, fiscalizador] + ANT_htdf[hogar, hora, dia, fiscalizador] 
                  for fiscalizador in fiscalizadores) <= 1 for hogar in hogares.keys() for hora in horas for dia in dias), name="tipo_visita")
...
string_resultado = f"resultados_{test_counter}.sol"
next(test_counter)
ruta_resultados = os.path.join("Resultados", "resultados.sol")
